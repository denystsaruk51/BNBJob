...
import {styles} from '../styles/recruitPresentation.style';
...
class RecruitPresentation extends Component {
  constructor(props) {
    super(props);
    ...
    this.gotoRegistrationRecruiterType=this.gotoRegistrationRecruiterType.bind(this);
    this.gotoRegistrationRecruiterTypeWithFacebook=this.gotoRegistrationRecruiterTypeWithFacebook.bind(this);
  ...
  gotoLogin() {
    Actions.login({userType: 'Recruit'});
  }
  gotoRegistrationRecruiterType() {
     Actions.registrationRecruiterType({fbEmail:'', fbPicture:'', fbFirstName:'', fbLastName:'', fbName:'', fbId:''});
  }
  gotoRegistrationRecruiterTypeWithFacebook() {
    ...
    SignInUser(fbEmail, password)
      .then((res) => {
        ...
        if(res.success === true) {
          ...
          this.gotoRegistrationRecruiterTypeWithFacebook();
        }
      })
      .catch((reason) => {
        this.setState({ isLoading: false });
      ...
      <View style={styles.container}>
        ...
        <Swiper style={styles.wrapper} height={240} loop={false}
          ...
          <View style={styles.slide}>
            <Image resizeMode='contain' style={styles.image} source={require('../image/img_presentation1_recruit.png')} />
            <Text style={styles.title}>{I18n.t('Video')}</Text>
            <Text style={styles.description}>{I18n.t('Watch the promo of the candidates by video, call candidates by see and video')}</Text>
          </View>
          <View style={styles.slide}>
            ...
            <Text style={styles.description}>{I18n.t('Find candidates in less than 90 minutes')}</Text>
          </View>
          <View style={styles.slide}>
            <Image resizeMode='contain' style={styles.image} source={require('../image/img_presentation3_recruiter.png')} />
            <Text style={styles.title}>{I18n.t('Good candidates')}</Text>
            <Text style={styles.description}>{I18n.t('Do not waste time BNBjob takes care of choosing the best candidates')}</Text>
          </View>
        </Swiper>
        ...
        <View style={styles.buttonGroup}>
          <View style={styles.redGroup}>
            ...
            <View style={styles.redBorderButtonView}>
              <TouchableOpacity onPress={this.gotoRegistrationRecruiterType}>
                <View>
...
export default connect(({routes}) => ({routes}))(RecruitPresentation);