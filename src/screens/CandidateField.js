...
import {styles} from '../styles/candidateField.style';
...
class CandidateField extends Component {
  constructor(props){
    super(props);
  ...
  onSkip(){
    Actions.reset("candidateHomePopup");
  }

  gotoSubField() {
    Actions.candidateSubField({text: 'Candidate Sub Field'});
  }
  ...
  handleItemClick(fieldItem, index) {
    ...
    Actions.candidateSubField({field: fieldItem});
  }
...
export default connect(({routes}) => ({routes}))(CandidateField)